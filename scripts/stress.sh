...
reset_tables() {
  echo "⚠ resetting warehouse tables (processed_events, event_type_counts_5m)…"
  if ! "$SCRIPT_DIR/sql.sh" -q -c \
      "TRUNCATE processed_events, event_type_counts_5m;" >/dev/null 2>&1; then
...
# event_type_counts_5m table is a 5-minute tumbling window that has not fired
...
verify_counts() {
  ...
  echo "  note:    event_type_counts_5m not verified — 5-min tumbling window; FFU"
  echo "─────────────────────────────────────────────"
}
