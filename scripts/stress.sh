...
reset_tables() {
  echo "⚠ resetting warehouse tables (processed_events, event_type_counts_agg, image_size_buckets_agg)…"
  if ! "$SCRIPT_DIR/sql.sh" -q -c \
      "TRUNCATE processed_events, event_type_counts_agg, image_size_buckets_agg;" >/dev/null 2>&1; then
...
# event_type_counts_agg and image_size_buckets_agg tables are tumbling windows
...
verify_counts() {
  ...
  echo "  note:    event_type_counts_agg, image_size_buckets_agg not verified — tumbling windows; FFU"
  echo "─────────────────────────────────────────────"
}
