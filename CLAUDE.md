...
- **Image size histogram**: image byte size is determined in one place — `MinioAsyncImageFunction` (fetched body length on the URL path, `statObject` size on the backend-upload passthrough) — and carried on `EnrichResult` to a side output that `buildImageSizeBuckets` windows into `image_size_buckets_5m`, reusing the per-type count sink path.
